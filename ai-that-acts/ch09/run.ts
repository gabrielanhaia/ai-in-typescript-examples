...
//
// The chapter prints the five lines with `fromRequest`, `customerId` and
// `token` already in scope. Here they come from somewhere, so the file runs:
// there is no HTTP request in front of this one, so `fromRequest` is a
// controller nobody pulls.
//
...
const session = sessionFor({ customerId, token, signal });
