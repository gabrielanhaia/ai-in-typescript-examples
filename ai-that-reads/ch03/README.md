...
- **`sourceId` is the path the loader was handed, not a corpus-relative path.** Driven from the package root these loaders produce `corpus/markdown/warranty-policy.md`, while chapter 13's `scanCorpus`, `corpus/questions.jsonl` and the field's own comment in `document.ts` all use `markdown/warranty-policy.md`. In the live pipeline the `source_id` column wins, because `ch13/store.ts` writes it from `scanCorpus`; it is the copy inside the metadata JSON that carries the extra segment. See [`../ch12/README.md`](../ch12/README.md).
